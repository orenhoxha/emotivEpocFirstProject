        ...
        private void StreamDataReceived(object sender, StreamDataEventArgs evt)
        {
            ...
            switch (streamType)
            {
                ...
                case (int)StreamID.PERF_METRICS_STREAM:
                    ...
                    break;
                case (int)StreamID.MENTAL_CMD_DATA_STREAM://this case is implemented by us
                    Console.WriteLine("Mental command data received");
                    ArrayList comData = new ArrayList();

                    JArray jComData = (JArray)evt.Data["com"];
                    foreach (var item in jComData)
                    {
                        comData.Add((string)item);
                    }
                    if (comData.Count > 0)
                    {
